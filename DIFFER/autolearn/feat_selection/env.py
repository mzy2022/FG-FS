...
from DIFFER.autolearn.data import Dataset
from DIFFER.autolearn.utils import timeit, log as logger
from DIFFER.autolearn.metrics import r2_score
from DIFFER.autolearn.feat_selection.nfo.search_space import *
...
class Environment:
    ...
    def get_feat(self, feat):
        feat = int(feat)
        return np.asarray(self.dataset.instances[feat].values, dtype=np.float64)
...
class SklearnEnv(Environment):
    ...
    def sklearn_evaluate(self, feats):
        ...
        if self.dataset.task == 'regression':
            ...
            score = cross_val_score(model, x, y, scoring=rae_score, cv=int(self.cv)).mean()
        elif self.dataset.task == 'classification':
            if self.model == 'LR':
