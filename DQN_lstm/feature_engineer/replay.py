    ...
    def sample(self,args,device):
        ...
        for i in range(args.episodes):
            ...
            for j in sample_index:
                state = self.memory[j][i][0].detach()
                action = self.memory[j][i][1]
                reward = self.memory[j][i][2]
                state_ = self.memory[j][i][3].detach()
                action_ = self.memory[j][i][4]
                res_h_c_t_list = [self.memory[j][i][5][0][0].detach(),self.memory[j][i][5][0][1].detach()]
                res_h_c_t_list_ = [self.memory[j][i][6][0][0].detach(),self.memory[j][i][6][0][1].detach()]
                states.append(state)
                actions.append(action)
                rewards.append(reward)
                states_.append(state_)
                actions_.append(action_)
                res_h_c_t_lists.append(res_h_c_t_list)
                res_h_c_t_lists_.append(res_h_c_t_list_)
            ...
            worker_list.append((states, actions, rewards, states_, actions_, res_h_c_t_lists, res_h_c_t_lists_))
        return worker_list
    ...
    def sample(self,args,device):
        ...
        for i in range(args.episodes):
            ...
            for j in sample_index:
                ...
                action_ = self.memory[j][i][4]
                res_h_c_t_list = self.memory[j][i][5]
                res_h_c_t_list_ = self.memory[j][i][6]
                states.append(state)
                actions.append(action)
                rewards.append(reward)
                states_.append(state_)
            ...
            states_ = states_.detach()
            actions_ = actions_.detach()
            res_h_c_t_lists = res_h_c_t_lists.detach()
            res_h_c_t_lists_ = res_h_c_t_lists_.detach()
            worker_list.append((states, actions, reward, states_, actions_,res_h_c_t_lists,res_h_c_t_lists_))
        return worker_list
