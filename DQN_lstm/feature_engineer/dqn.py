...
class DQN_ops(object):
    ...
    def choose_action_ops(self, input, for_next,steps_done,res_h_c_t_list):
        actions_ops = []

        self.agent1.train()
        ops_vals, state,res_h_c_t_list,ori_res_h_c_t_list = self.agent1(input.to(self.device), for_next,res_h_c_t_list)
        ...
        return actions_ops, state,res_h_c_t_list,ori_res_h_c_t_list


    def store_transition(self,args,workers,i):
        store_ops_list = []
        for num, worker in enumerate(workers):
            ...
            res_h_c_t = worker.ori_res_h_c_t_lists[i]
            res_h_c_t_ = worker.res_h_c_t_lists[i]
            store_ops_list.append([states_ops,ops,reward,states_ops_,ops_,res_h_c_t,res_h_c_t_])
    ...
    def learn(self, args, workers,device):
        if self.memory.memory_counter >= self.memory.memory_capacity:
            ...
            for memory_list in worker_lists:
                # with torch.no_grad():
                state,action,reward,state_,action_,h_c,h_c_ = memory_list
                for i in range(args.memory):
                    q_next = self.agent1.target_net(state_[i],[h_c_[i]])[1]
                    q_target = reward[i] + self.gamma * torch.max(q_next, dim=-1)[0]
                    q_eval = self.agent1.eval_net(state[i],[h_c[i]])[1]
                    yyy = torch.tensor(action[i],dtype=torch.long).unsqueeze(0).to(device)
                    q_eval_selected = q_eval.gather(1, yyy).squeeze(0)
                    # q_target = q_target.detach()
                    loss = loss + self.loss_func(q_eval_selected, q_target)

            loss /= len(worker_lists) * args.batch_size
