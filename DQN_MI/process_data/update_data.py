...
class Update_data():
    def __init__(self, state_c, state_d, pipline_data):
        ...
        # self.new_continuous = self.ori_cols_continuous.iloc[:,:-1]
        # self.new_combine = self.ori_cols_combine.iloc[:,:-1]
    ...
    def single_fe_operations(self):
        ...
        for index, operation in self.value_convert.items():
            ...
            if operation[0] == 6:
                ...
                #     self.Candidate_features[name] = new_fe
            # if operation[1] == 0:
            self.new_continuous[name] = new_fe.reshape(-1).copy()
            # elif operation[1] == 1:
        ...
        for i, feature_information in enumerate(feature_informations):
            ...
            if operations[i] == "combine":
                ...
                for actions in feature_information:
                    ...
                    ori_fe2 = self.ori_cols_combine.iloc[:, actions[1]]
                    name2 = ori_fe1.name + '_nunique_' + ori_fe2.name
                    new_fe = get_nunique_feature(ori_fe1, ori_fe2)

                    # self.Candidate_features[name] = new_fe
                    # if actions[2] == 0:
                    self.new_combine[name2] = new_fe.reshape(-1)
            # elif actions[2] == 1:
