...
from DQN_my3.process_data.utils_memory import get_pos_emb
...
class Actor1(nn.Module):
    def __init__(self, args, data_nums, feature_nums, operations_c, operations_d,d_model, d_k, d_v, d_ff, n_heads, device,
                 dropout=None):
        ...
        self.nums = max(operations_c,operations_d)
        self.eval_net = QNet_ops(d_model, self.nums)
        self.target_net = QNet_ops(d_model, self.nums)
    ...
    def forward(self, input, for_next, con_or_dis):
        ...
        # con_or_dis = torch.tensor(con_or_dis).to(self.device).to(torch.float32)
        # con_or_dis = con_or_dis.unsqueeze(-1)
        ...
        # data_reduction_dimension = data_reduction_dimension.squeeze(0)
        ...
        # data_reduction_dimension = torch.concat([data_reduction_dimension, con_or_dis], dim=1)
        # encoder_output = data_reduction_dimension.squeeze(0)[:-1]
        # data_reduction_dimension = data_reduction_dimension.unsqueeze(0)
        ...
        if for_next:
            ops_logits = self.target_net(encoder_mean)
        else:
            ops_logits = self.eval_net(encoder_mean)
        ops_logits = torch.where(torch.isnan(ops_logits), torch.full_like(ops_logits, 0), ops_logits)

        return ops_logits, encoder_mean
...
class Actor2(nn.Module):
    def __init__(self, args, operations_c,operations_d, hidden_size, d_model,n_c_features,n_d_features):
        super(Actor2, self).__init__()
        self.args = args
        self.nums = max(operations_c,operations_d)
        self.embedding = nn.Embedding(n_c_features*4+2*n_d_features+6, hidden_size)
        self.eval_net = QNet_otp(2*hidden_size + d_model, self.nums)
        self.target_net = QNet_otp(2*hidden_size + d_model, self.nums)
...
class Actor3(nn.Module):
    def __init__(self, args, operations_c,operations_d, hidden_size, d_model,feature_nums):
        super(Actor3, self).__init__()
        self.args = args
        self.nums = max(operations_c,operations_d)
        self.embedding = nn.Embedding(self.nums+1, hidden_size)
        self.eval_net = QNet_otp(hidden_size + d_model, feature_nums)
        self.target_net = QNet_otp(hidden_size + d_model, feature_nums)
...
class QNet_features(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=128, init_w=0.1, device=None):
        super(QNet_features, self).__init__()
        self.fc1 = nn.Linear(state_dim, hidden_dim)
        self.fc1.weight.data = fanin_init(self.fc1.weight.data.size())
        self.out = nn.Linear(hidden_dim, action_dim)
        self.out.weight.data.normal_(-init_w, init_w)
