...
class PPO(object):
    ...
    def choose_action_ops(self, input_c, step, epoch, ops):
        actions_ops = []
        log_ops_probs = []
        self.agent1.train()
        ops_softmax, state = self.agent1(input_c.to(self.device))
        ...
        for epoch in range(self.args.ppo_epochs):
            ...
            for worker_index, worker in enumerate(workers):
                ...
                for index, state in enumerate(states):
                    ...
                    ops_softmax, emb_state = self.agent1(state.to(self.device))
    ...
    def update_otp(self, workers):
        ...
        for epoch in range(self.args.ppo_epochs):
            ...
            for worker_index, worker in enumerate(workers):
                ...
                new_log_probs = []
                entropys = []
                for index, (action_ops,emb_state) in enumerate(zip(actions_ops,emb_states)):
                    action_ops = torch.tensor([item for item in action_ops]).to(self.device)

                    otp_softmax,all_embedding = self.agent2(action_ops,emb_state.to(self.device))
