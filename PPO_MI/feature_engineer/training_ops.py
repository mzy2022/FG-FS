...
from PPO_MI.fe_parsers import parse_actions
from PPO_MI.metric_evaluate import rae_score
from PPO_MI.model_evaluate import model_fuctions
from PPO_MI.process_data import Pipeline
from PPO_MI.process_data.utils_memory import remove_duplication
from PPO_MI.process_data.update_data import Update_data
def sample(args, ppo, pipline_args_train, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, epoch, i, device):
    ...
    for step in range(steps_num):
        ...
        steps.append(step)
        state = init_state
        actions_ops, log_ops_probs, emb_state = ppo.choose_action_ops(state, step, epoch,c_ops)
        ...
        if x_c.shape[0] != 0 and x_c.shape[1] >= 2 * n_c_features:
            selector = SelectKBest(mutual_info_regression, k=n_c_features).fit(x_c, df_t_norm)
            cols = selector.get_support()
            x_c = x_c.loc[:, cols]

        if x_d.shape[0] != 0 and x_d.shape[1] >= 2 * n_d_features:
            selector = SelectKBest(mutual_info_regression, k=n_d_features).fit(x_d, df_t_norm)
            cols = selector.get_support()
            x_d = x_d.loc[:, cols]
