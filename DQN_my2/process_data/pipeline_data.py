...
class Pipeline(object):
    ...
    def __init__(self, args, bins_thresh=8):
        ...
        self.combine = self.ori_dataframe[self.discrete_columns].copy()
            ...
            for col_index_tuple in combine_feature_tuples_list:
                ...
                if abs(x) < 0.1 and operation in ['subtract', 'divide']:
                    ...
                    if operation in ['add', 'multiply']:
                        ...
                        # if special_op != 1:
                        #     self.Candidate_features[name] = new_fe
                new_fe = np.nan_to_num(new_fe.reshape(-1)).astype(np.float32)
                new_fe = np.where(new_fe > 10000000, 0,new_fe)
                # self.continuous = self.continuous.copy()
                if special_op == 0:
                    self.continuous[name] = new_fe
                    self.specials['concat'].append([col1.values, new_fe])
                elif special_op == 2:
                    delete_index.append(col1_index)
                    self.specials['delete'].append([col1.values, new_fe])
                else:
                    self.continuous[col1.name] = new_fe
                    self.continuous = self.continuous.rename(columns={col1.name: name})
                    self.specials['replace'].append([col1.values, new_fe])
    ...
    def single_fe_operations(self):
        ...
        for index, operation in self.value_convert.items():
            ...
            if operation[1] == 0:
                ...
                self.specials['concat'].append([ori_col.values, new_fe])
            elif operation[1] == 1:
                self.continuous[ori_col.name] = new_fe
                self.continuous = self.continuous.rename(columns={ori_col.name: name})
                self.specials['replace'].append([ori_col.values, new_fe])
            else:
                delete_index.append(index)
                self.specials['delete'].append([ori_col.values, new_fe])
        # if 0 < len(delete_index) < self.continuous.shape[1]:
        #     self.continuous_drop_list.extend(delete_index)
        ...
        for i, feature_information in enumerate(feature_informations):
            ...
            if self.combine.shape[1] == 0:
                ...
                for actions in feature_information:
                    ...
                    if actions[2] == 0:
                        self.combine[name] = new_fe
                    elif actions[2] == 1:
                        self.combine[ori_fe1.name] = new_fe
