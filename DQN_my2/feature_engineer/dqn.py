...
class DQN_ops(object):
    def __init__(self, args, data_nums, feature_nums,operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads,
        ...
        self.agent1 = Actor1(args, data_nums, feature_nums,operations_c, operations_d,d_model, d_k, d_v, d_ff, n_heads, self.device,dropout=dropout).to(self.device)
...
class DQN_otp(object):
    def __init__(self, args,operations_c,operations_d,hidden_size,d_model,memory,device,EPS_START=0.9, EPS_END=0.05, EPS_DECAY=200):
        ...
        self.device = device
        self.memory = memory
        self.otp_nums = max(operations_c,operations_d)
        self.eps_end =EPS_END
        self.eps_start = EPS_START
        self.eps_decay = EPS_DECAY
        self.loss_func = nn.MSELoss()
        ...
        self.gamma = 0.99
        self.batch_size = 8
        self.agent2 = Actor2(args, self.otp_nums,hidden_size,d_model).to(self.device)
