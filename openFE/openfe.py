...
class OpenFE:
    ...
    def transform(self, X_train, X_test, new_features_list, n_jobs, name=""):
        ...
        results = []
        aaa = len(new_features_list)
        for num,feature in enumerate(new_features_list):
            print(f"[{num}/{aaa}]")
...
if __name__ == '__main__':
    ...
    n_jobs = 8
    data = pd.read_csv("Openml_586.csv")
    task = 'reg'
    X = data.iloc[:, :-1]
    y = data.iloc[:, -1]
    print(X.shape)
    # old_score = get_score(X, y, task=task)
    ...
    if task == 'reg':
        ...
        for train, test in kf.split(X):
            ...
            y_predict = reg.predict(X_test)
            rae_list.append(1 - relative_absolute_error(y_test, y_predict))
        print(f"old_score:{np.mean(rae_list)}")
    else:
        clf = RandomForestClassifier(random_state=0)
        f1_list = []
        skf = StratifiedKFold(n_splits=5, random_state=0, shuffle=True)
        for train, test in skf.split(X, y):
            ...
            y_predict = clf.predict(X_test)
            f1_list.append(f1_score(y_test, y_predict, average='weighted'))
        print(f"old_score:{np.mean(f1_list)}")
    train_x, test_x, train_y, test_y = train_test_split(X, y, test_size=0.2, random_state=1)
    ofe = OpenFE()
    ofe.fit(data=train_x, label=train_y, n_jobs=n_jobs, task="regression")
    #regression，classification
    train_x, test_x = ofe.transform(train_x, test_x, ofe.new_features_list, n_jobs=n_jobs)
    ...
    if task == 'reg':
        ...
        for train, test in kf.split(X):
            ...
            y_predict = reg.predict(X_test)
            rae_list.append(1 - relative_absolute_error(y_test, y_predict))
        print(f"new_score:{np.mean(rae_list)}")
    else:
        clf = RandomForestClassifier(random_state=0)
        f1_list = []
        skf = StratifiedKFold(n_splits=5, random_state=0, shuffle=True)
        for train, test in skf.split(X, y):
            ...
            y_predict = clf.predict(X_test)
            f1_list.append(f1_score(y_test, y_predict, average='weighted'))
        print(f"new_score:{np.mean(f1_list)}")
    exp_log_dir = 'qwe'
    ...
    # 在日志记录器中使用logging.info()记录消息，消息将被写入指定的文件中
    ...
    for feature in ofe.new_features_list:
        ...
        logging.info(f'new_f : {tree_to_formula(feature)}')
