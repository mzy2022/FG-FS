...
class Pipline(object):
    ...
    def noaction_baseline(self,ori_dataframe):
        ...
        if self.do_onehot:
            if len(self.discrete_columns) > 0:
                ...
                res_colname_list = self.continuous_columns + onehot_col_list
    ...
    def process_cycle_ops_list(self,cycle_ops_list, continuous_df, discrete_df):
        replace_param_list, concat_param_list, replace_param_list_combine, concat_param_list_combine = self.get_param_list(
            cycle_ops_list, continuous_df, discrete_df, self.continuous_columns)
    ...
    def create_action_fes(self, actions, ori_dataframe, task_type='classifier', target_col=None, train=True,
                          train_params=None, test=True):
        self.train = train
        self.test = test
        if self.train == True:
    ...
    #
    # def single_bins_discrete(self, ori_columns, bins, ori_fe):
    #     if self.train:
    #         if len(ori_fe.unique()) > bins:
    #             ori_fe_bins = self.fes_eng.discrete_freq_bins(ori_fe, bins, fe_name=ori_columns)
    #         else:
...
if __name__ == '__main__':
    df = pd.DataFrame(
        [[1, 2, 3, 6, 7], [3, 5, 3, 6, 4], [1, 5, 9, 8, 7], [3, 3, 2, 5, 8], [4, 5, 6, 7, 8], [9, 5, 2, 1, 6],
         [6, 4, 2, 7, 3]], columns=['A', 'b', 'C', 'd', 'label'])
    ...
    # res, label = pipline_ins.create_action_fes(actions, df)
