...
class Controller(nn.Module):
    def __init__(self,args):
        ...
        if self.with_hyper_param_search:
            self.hyper_param_decoder = nn.Linear(self.hidden_size, self.model_param_num * self.handled_space_len)
        else:
    ...
    def sample(self):
        ...
        if self.with_hyper_param_search:
            hyper_param_logits = self.forward(input_data, hc_list, is_hyper_param=True)
            hyper_param_logits = hyper_param_logits.view(self.model_param_num, self.handled_space_len)
            hyper_param_info = self.sample_cycle_component(hyper_param_logits, 'hyper_param')
    ...
    def get_prod(self, sample_history, detach_bool=False):
        ...
        if self.with_hyper_param_search:
            ...
            hyper_param_info = self.prod_cycle_component(hyper_param_logits, hp_action_index_local)
            return_dict['hyper_parameter'] = hyper_param_info
        else:
            return_dict['hyper_parameter'] = None
        return return_dict
