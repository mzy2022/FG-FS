...
class DatasetSplit(object):
    ...
    def split_dataset_with_ratio(self, df, test_rate, random_state=1):
        ...
        if self.task_type == 'classifier':
            x_train, x_test, y_train, y_test = train_test_split(df_feature,df_target,test_size=test_rate,shuffle=True, stratify=df_target, random_state=random_state)
        elif self.task_type == 'regression':
            x_train, x_test, y_train, y_test = train_test_split(df_feature, df_target, test_size=test_rate,shuffle=True, random_state=random_state)
        else:
            logging.info('task_type error')
            x_test = x_train = y_train = y_test = None
        # feature target merge
        train_data = x_train.merge(pd.DataFrame(y_train),left_index=True,right_index=True, how='left')
    ...
    def save_to_csv(self, train_data, test_data):
        save_path = '{}_{}'.format(self.dataset_name, time.strftime("%Y%m%d-%H%M%S"))
    ...
    def load_all_data(self):
        ...
        all_data = all_data[self.args.continuous_col+self.args.discrete_col +[self.args.target_col]]
