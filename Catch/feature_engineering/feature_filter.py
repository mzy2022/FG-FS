...
class FeatureFilterMath(object):
    ...
    def chi2_filter(self,data,label,p_threshold=0.05):
        ...
        zerostd_idx = [(idx, global_idx) for idx, global_idx in enumerate(discrete_idx) if np.std(discrete_fes[:, idx]) == 0]
        ...
        if discrete_fes_nozerostd.shape[1] == 0:
            ...
            delete_p_idx = [(idx, global_idx) for idx, global_idx in enumerate(remain_discrete_idx) if pvalues_chi[idx] > p_threshold]
