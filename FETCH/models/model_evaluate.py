...
def lr_classify(n_jobs, penalty='l2'):
    model = LogisticRegression(class_weight='balanced', n_jobs=n_jobs, tol=0.0005, C=0.1, max_iter=10000,
                               penalty=penalty)
    return model
...
def rf_pre_classify(n_jobs):
    model = RandomForestClassifier(n_estimators=600, max_depth=8, n_jobs=n_jobs, class_weight='balanced',
                                   random_state=42)
    return model
...
model_fuctions = {"lr_regression": lr_regression, "lr_classify": lr_classify, "rf_regression": rf_regression,
                  ...
                  "cat_classify": cat_classify, "rf_pre_regression": rf_pre_regression,
                  "rf_pre_classify": rf_pre_classify, }
