"""
detect_TIMESTAMP(df, col)函数用于检测给定列是否包含时间戳数据。
detect_DATETIME(df, col)函数用于检测给定列是否包含日期时间数据
get_data_type(df, col)函数根据列的数据类型来判断其特征类型
Feature_type_recognition类拥有一个fit方法，它接收一个数据框（df）作为输入，并对数据框的每一列进行特征类型识别
"""
...
def detect_TIMESTAMP(df, col):
    try:
        ...
        tx_max = int(float(df.loc[~(df[col] == '') & (df[col].notnull()), col].max()))
