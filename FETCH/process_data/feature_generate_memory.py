...
# 对离散特征进行分箱处理
def binning_for_discrete(ori_fe: np.array, bins: int, mode: str, label: np.array, col_index: int = None,
                         memory: Memory = None, isvalid: bool = False):
...
# 用于使用决策树进行分箱操作
def binning_with_tree(ori_fe: np.array, label: np.array, col_index: int = None, memory: Memory = None,
                      isvalid: bool = False):
    if isvalid:
...
def normalization(col: list or np.ndarray, col_index: int = None, memory: Memory = None,
...
def binning(ori_fe: np.array, bins: int, col_index: int, method: str = 'frequency', memory: Memory = None,
            isvalid: bool = False):
...
def dfs(combine_categories_list: list, feasible_value: str, feasible_values: list) -> None:
...
def features_combine_ori(ori_fes: np.array, indexs: list, memory: Memory = None, isvalid: bool = False) -> np.array:
