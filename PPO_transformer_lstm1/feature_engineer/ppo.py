...
class PPO(object):
    def __init__(self, args, data_nums, feature_nums,operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads,
        ...
        self.actor = Actor(args, data_nums,feature_nums, operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads,
    ...
    def choose_action(self, input_c, step, epoch, ops,res_h_c_t_list):
        ...
        log_ops_probs = []
        self.actor.train()
        ops_softmax,res_h_c_t_list = self.actor(input_c.to(self.device), step,res_h_c_t_list)
        ...
        return actions_ops, log_ops_probs,res_h_c_t_list
        ...
        for epoch in range(self.args.ppo_epochs):
            ...
            for worker_index, worker in enumerate(workers):
                ...
                for index, state in enumerate(states):
                    ...
                    if index == 0:
                        res_h_c_t_list = None
                    ops_softmax,res_h_c_t_list = self.actor(state.to(self.device), step,res_h_c_t_list)
