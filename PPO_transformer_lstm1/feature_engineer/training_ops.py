...
from PPO_transformer_lstm1.fe_parsers import parse_actions
from PPO_transformer_lstm1.metric_evaluate import rae_score
from PPO_transformer_lstm1.model_evaluate import model_fuctions
from PPO_transformer_lstm1.process_data import Pipeline
from PPO_transformer_lstm1.process_data.utils_memory import remove_duplication
from PPO_transformer_lstm1.process_data.update_data import Update_data
def sample(args, ppo, pipline_args_train, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, epoch, i, device):
    ...
    final_features_c = []
    final_features_d = []
    past_x_c = df_c_encode.drop(df_c_encode.columns[-1], axis=1)
    past_x_d = df_d_encode.drop(df_d_encode.columns[-1], axis=1)
    ...
    for step in range(steps_num):
        ...
        past_x_d = pd.concat([past_x_d, df_c_encode], axis=1)
        ...
        if x_d.shape[0] == 0:
            x_encode_c = np.hstack((past_x_c,x_c,df_t_norm.values.reshape(-1, 1)))
        elif x_c.shape[0] == 0:
            x_encode_c = np.hstack((past_x_d,x_d, df_t_norm.values.reshape(-1, 1)))
        else:
            x_encode_c = np.hstack((past_x_c,x_c, past_x_d,x_d, df_t_norm.values.reshape(-1, 1)))
        ...
        final_features_c.append(past_x_c)
        final_features_d.append(past_x_d)
    ...
    worker.final_features_c = final_features_c
    worker.final_features_d = final_features_d
    worker.ff = ff
    return worker
...
def multiprocess_reward(args, worker, c_columns, d_columns, scores_b, mode, model, metric, x_d_onehot, y,df_d_labelencode):
    ...
    for step in range(args.steps_num):
        x_c = worker.final_features_c[step]
        x_c = x_c.drop(x_c.columns[-1], axis=1)
        x_d = worker.final_features_d[step]
    ...
    worker.features_c = x_cs
    worker.features_d = x_ds
