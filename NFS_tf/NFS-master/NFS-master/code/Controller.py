...
class Controller:
    ...
    def _create_rnn(self):
        ...
        for i in range(self.num_feature):
            self.rnns['rnn%d' % i] = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(
                num_units=self.num_op, name='rnn%d' % i)

    def _create_placeholder(self):
        self.concat_action = tf.compat.v1.placeholder(tf.int32,
        ...
        self.state = tf.compat.v1.placeholder(tf.int32,
                                    shape=[None, self.num_action], name='state')
        self.value = tf.compat.v1.placeholder(tf.float32,
    ...
    def _create_variable(self):
        ...
        self.value_estimator = Sequential([
            ...
            Activation('tanh'),
            Dense(1)])
        self.value_optimizer = Adam(lr=self.lr_value)
    ...
    def _create_loss(self):
        ...
        for batch_count in range(self.num_batch):
            ...
            loss_batch = tf.reduce_sum(-tf..mathlog(pick_action_prob) * reward)
