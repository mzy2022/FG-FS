...
def parse_args():
	...
	parser.add_argument('--num_op_unary', type=int,default=4, help='unary operation num')
	parser.add_argument('--num_op_binary', type=int,default=5, help='binary operation num')
	parser.add_argument('--max_order', type=int,default=5, help='max order of feature operation')
	parser.add_argument('--num_batch', type=int,default=5, help='batch num')
	parser.add_argument('--optimizer', nargs='?',default='adam', help='choose an optimizer')
	parser.add_argument('--lr', type=float,default=0.01, help='set learning rate')
	parser.add_argument('--epochs', type=int,default=100, help='training epochs')
	parser.add_argument('--evaluate', nargs='?',default='f_score', help='choose evaluation method')
	parser.add_argument('--task', nargs='?',default='classification', help='choose between classification and regression')
	parser.add_argument('--dataset', nargs='?',default='credit_g', help='choose dataset to run')
	parser.add_argument('--model', nargs='?',default='RF', help='choose a model')
	parser.add_argument('--alpha', type=float,default=0.99, help='set discouont factor')
	parser.add_argument('--lr_value', type=float,default=1e-3, help='value network learning rate')
	parser.add_argument('--RL_model', nargs='?',default='PG', help='choose RL model, PG or AC')
	parser.add_argument('--reg', type=float,default=1e-5, help='regularization')
	parser.add_argument('--controller', nargs='?',default='rnn', help='choose a controller')
	parser.add_argument('--num_random_sample', type=int,default=5, help='sample num of random beseline')
	parser.add_argument('--lambd', type=float,default=0.4, help='TD lambd')
	parser.add_argument('--multiprocessing', type=bool,default=False, help='whether get reward using multiprocess')
	parser.add_argument('--package', nargs='?',default='sklearn', help='choose sklearn or weka to evaluate')
	return parser.parse_args()

args = parse_args()
def get_reword(actions,args):
	global path, method, origin_result
	global best_score
	best_score = origin_result
	...
	if method == 'train':
		...
		y = X[X.columns[-1]]
		gene_df = pd.DataFrame(X).copy()
		del X[X.columns[-1]]
		result = evaluate(X, y, args)
		if result > best_score:
			best_score = result
			gene_df.to_csv('Catch_log.csv',index=False)
...
def random_run(num_random_sample, model, l=None, p=None):
	...
	if args.multiprocessing:
		...
		for sample in samples:
			res.append(get_reword(sample,args))
		...
		for epoch_count in range(args.epochs):
			...
			if args.multiprocessing:
				...
				for action in concat_action:
					rewards.append(get_reword(action,args))
			...
			if args.multiprocessing:
				...
				results = []
				for action in concat_action:
					results.append(get_reword(action,args))
			...
			model_result = max(model_result, get_reword(best_action,args))
