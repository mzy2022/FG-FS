...
class BigFeat:
    ...
    def fit(self,X,y,gen_size=5,random_state=0, iterations=50,estimator='avg',feat_imps = True, split_feats = None, check_corr= True, selection = 'stability', combine_res = True):
...
if __name__ == '__main__':
    ...
    datasets = [
        ...
        ("data/spectf.csv","label")

    ]
    for each in datasets:
        X_train, X_test, y_train, y_test = run_tst(each[0], each[1], random_state=0)
        res = bf.fit(X_train, y_train, gen_size=10, random_state=0, iterations=120, estimator='avg', feat_imps=True,
                     split_feats=None, check_corr=True, selection='stability', combine_res=True)
        print(each[0])
        ...
        print(f"old{f1_score(y_test, y_predict, average='weighted')}")
        clf = RandomForestClassifier(random_state=0).fit(bf.transform(X_train), y_train)
        y_pred_bf = clf.predict(bf.transform(X_test))
        print(f"new{f1_score(y_test, y_pred_bf, average='weighted')}")
