...
class Actor1(nn.Module):
    def __init__(self, args, data_nums, feature_nums,operations_c,d_model, d_k, d_v, d_ff, n_heads, dropout=None):
        ...
        self.eval_net = QNet_ops(d_model, operations_c)
        self.target_net = QNet_ops(d_model, operations_c)
        self.layernorm = nn.LayerNorm(data_nums)
        self.feature_nums = feature_nums

    def forward(self, input, for_next):
...
class Actor2(nn.Module):
    def __init__(self, args,operations_c, hidden_size):
        super(Actor2, self).__init__()
        self.args = args
        self.embedding = nn.Embedding(operations_c, hidden_size)
        self.eval_net =QNet_otp(hidden_size, operations_c)
        self.target_net = QNet_otp(hidden_size, operations_c)
    ...
    def forward(self, input,for_next):
        ...
        if for_next:
            otp_logits = self.eval_net(embedding_output)
        else:
            otp_logits = self.target_net(embedding_output)
