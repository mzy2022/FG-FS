...
from DQN_1agent.metric_evaluate import rae_score
from DQN_1agent.model_evaluate import model_fuctions
from DQN_1agent.process_data.pipeline_data import Pipeline
from DQN_1agent.fe_parsers import parse_actions
from DQN_1agent.process_data.update_data import Update_data
...
def sample(args, dqn_ops, pipline_data, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, step, workers, device,
    ...
    if step == 0:
        for _ in range(args.episodes):
            ...
            pipline_ffs.append(Update_data(pipline_data['continuous_data'].drop(pipline_data['continuous_data'].columns[-1],axis=1),pipline_data['discrete_data'].drop(pipline_data['discrete_data'].columns[-1],axis=1),pipline_data))
    else:
    ...
    for i in range(args.episodes):
        ...
        actions_ops, states_ops = dqn_ops.choose_action_ops(states[i], for_next, steps_done)
...
def sample_update(args, pipline_data, dqn_ops, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, workers,steps_done,device):
    ...
    for i in range(args.episodes):
        ...
        actions_ops, states_ops = dqn_ops.choose_action_ops(states_[i], for_next, steps_done)
        fe = parse_actions(actions_ops, c_ops, d_ops, df_c_encode, df_d_encode, n_c_features, n_d_features)
