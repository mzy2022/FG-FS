    ...
    def fit_attention(self, args):
        ...
        for epoch in range(args.epochs):
            ...
            for i in range(args.steps_num):
                ...
                workers_dqn = sample(args, self.dqn_ops,pipline_data, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, i,workers, self.device,self.steps_done)
                # 得到reward
                workers_reward = multiprocess_reward(args, workers_dqn, scores_b, mode,model, metric,df_t.values)
                # 得到经过两个agent之后的actions_，states_
                workers_dqn_ = sample_update(args, pipline_data,self.dqn_ops,df_c_encode, df_d_encode,df_t_norm, c_ops,d_ops,workers_reward,self.steps_done,self.device)
