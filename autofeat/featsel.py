...
def _noise_filtering( X: np.ndarray, target: np.ndarray, good_cols: list | None = None, problem_type: str = "regression") -> list:
    """
...
def _select_features_1run(df: pd.DataFrame, target: np.ndarray, problem_type: str = "regression", verbose: int = 0) -> list:
    ...
    if other_cols:
        ...
        for i in range(n_splits):
            ...
            if verbose > 0:
                print(f"[featsel]\t Split {i + 1:2}/{n_splits}: {len(good_cols_set):3} candidate features identified.",end="\r")
