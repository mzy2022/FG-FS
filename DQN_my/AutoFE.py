...
class AutoFE:
    ...
    def fit_attention(self, args):
        ...
        self.dqn_otp = DQN_otp(args, operations_c,hidden_size,d_model,self.memory_otp, self.device)
        self.steps_done = 0
        ...
        for epoch in range(args.epochs):
            ...
            for num, worker in enumerate(workers):
                worker_dqn_ = sample_update(args, pipline_data,self.dqn_ops,self.dqn_otp,df_c_encode, df_d_encode,df_t_norm, c_ops,d_ops,worker,self.device)
                workers[num] = worker_dqn_


            self.dqn_ops.store_transition(args,workers)
            self.dqn_otp.store_transition(args,workers)
            ...
            for i in range(5):
                logging.info(f"epoch{epoch}_top_{i + 1}:score:{self.workers_top5[i].scores.mean()}")


            self.dqn_ops.learn(args,workers,self.device)
            self.dqn_otp.learn(args,workers,self.device)
