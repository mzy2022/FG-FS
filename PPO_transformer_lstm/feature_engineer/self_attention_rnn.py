...
class Actor(nn.Module):
    def __init__(self, args, data_nums, operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads, rnn_hidden_size,
        ...
        self.select_operation = SelectOperations(d_model, rnn_hidden_size, operations_c)
    ...
    def forward(self, input, step, res_h_c_t_list):
        ...
        otp_softmax = torch.softmax(otp_logits, dim=-1)

        return ops_softmax, otp_softmax, res_h_c_t_list
...
class SelectOperations(nn.Module):
    def __init__(self, d_model, hidden_size, num_ops):
        self.hidden_size = hidden_size
        super().__init__()
        self.otp_rnn = nn.LSTMCell(d_model, hidden_size)
        #  LSTMCell
        self.ops_rnn = nn.LSTMCell(d_model, hidden_size)
        # M operation embedding
        self.opt_decoder = nn.Linear(hidden_size, num_ops)
        # T operation embedding
        self.ops_decoder = nn.Linear(hidden_size, num_ops)

        state_size = (1, hidden_size)
    ...
    def forward(self, input_data, h_c_t_list):
        ...
        otp_h_t, otp_c_t = h_c_t_list[0]
        ops_h_t, ops_c_t = h_c_t_list[1]
        for i in range(input_data.shape[0] - 1, -1, -1):
            ...
            ops_h_t, ops_c_t = self.ops_rnn(otp_h_t, (ops_h_t, ops_c_t))
            output_otp.append(otp_h_t)
            output_ops.append(ops_h_t)

        output_otp = torch.stack(output_otp, dim=0).squeeze(1)
        output_ops = torch.stack(output_ops, dim=0).squeeze(1)
        otp_logits = self.opt_decoder(output_otp)
        ops_logits = self.ops_decoder(output_ops)
