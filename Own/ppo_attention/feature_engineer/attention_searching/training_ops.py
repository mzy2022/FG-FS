...
def sample(args, ppo, pipline_args_train, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, epoch, i, device):
    ...
    state_size = (1, 1, 128)
    init_ops_h = nn.Parameter(torch.zeros(state_size)).to('cuda')
    init_ops_c = nn.Parameter(torch.zeros(state_size)).to('cuda')
    ...
    for step in range(steps_num):
        ...
        if df_c_encode.shape[0] > 1:
            ...
            if step == 0:
                h = init_ops_h
                c = init_ops_c
            actions, log_probs, action_softmax,h,c = ppo.choose_action_c(state_c, step, epoch, c_ops,sample_rule,h,c)
        ...
        if args.combine:
            ...
            if step == 0:
                h = init_ops_h
                c = init_ops_c
            actions, log_probs,action_softmax,h,c = ppo.choose_action_d(state_d, step, epoch, c_ops, sample_rule,h,c)
            logging.debug(f'Start parse_actions...')
...
def multiprocess_reward(args, worker_c, worker_d, c_columns, d_columns, scores_b, mode, model, metric, x_d_onehot, y,
    ...
    if args.combine:
        for step in range(args.steps_num):
            ...
            if len(x_c.shape) != 1 and len(x_d.shape) != 1:
                new_fe_num = x_c.shape[1] + x_d.shape[1]
            elif len(x_c.shape) == 1 and len(x_d.shape) != 1:
                new_fe_num = 1 + x_d.shape[1]
            elif len(x_c.shape) != 1 and len(x_d.shape) == 1:
                new_fe_num = 1 + x_c.shape[1]
            else:
                new_fe_num = 2
