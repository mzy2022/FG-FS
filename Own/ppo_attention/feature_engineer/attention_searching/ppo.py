...
class PPO(object):
    def __init__(self, args, data_nums,data_features,operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads,
        ...
        if operations_c:
            self.actor_c = Actor(args, data_nums,data_features, operations_c, d_model, d_k, d_v, d_ff, n_heads, dropout=dropout, enc_load_pth=args.enc_c_pth).to(self.device)
            self.actor_c_opt = optim.Adam(params=self.actor_c.parameters(), lr=args.lr)
        if operations_d:
            self.actor_d = Actor(args, data_nums,data_features, operations_d, d_model, d_k, d_v, d_ff, n_heads, dropout=dropout, enc_load_pth=args.enc_d_pth).to(self.device)
        ...
        state_size = (1, 1, 128)
        self.init_ops_h = nn.Parameter(torch.zeros(state_size)).to('cuda')
        self.init_ops_c = nn.Parameter(torch.zeros(state_size)).to('cuda')

    def choose_action_c(self, input_c, step, epoch, ops, sample_rule,h,c):
        actions = []
        log_probs = []

        self.actor_c.train()
        input_c = input_c.permute(1,0)
        input_c = input_c[:,:-1]
        action_softmax,h,c = self.actor_c(None,input_c.to(self.device),h , c, step)
        if action_softmax.dim() == 1:
            action_softmax = action_softmax.unsqueeze(0)
        index_none = []
        ...
        return actions, log_probs, action_softmax,h,c

    def choose_action_d(self, input_d, step, epoch, ops, sample_rule,h,c):
        actions = []
        log_probs = []

        self.actor_d.train()
        input_d = input_d.permute(1,0)
        input_d = input_d[:, :-1]
        if step == 0:
            h = self.init_ops_h
            c = self.init_ops_c
        action_softmax,h,c = self.actor_d(input_d.to(self.device),None,h,c, step)
        if action_softmax.dim() == 1:
            action_softmax = action_softmax.unsqueeze(0)
        index_none = []
        for index, out in enumerate(action_softmax):
            ...
            for worker_index, worker in enumerate(workers_c):
                ...
                for index, state in enumerate(states):
                    ...
                    if step == 0:
                        h = self.init_ops_h
                        c = self.init_ops_c
                    action_softmax,h,c = self.actor_c(None,state.to(self.device),h,c, step)
                    if action_softmax.dim() == 1:
                        action_softmax = action_softmax.unsqueeze(0)
            ...
            for worker_index, worker in enumerate(workers_d):
                ...
                for index, state in enumerate(states):
                    ...
                    if step == 0:
                        h = self.init_ops_h
                        c = self.init_ops_c
                    action_softmax,h,c = self.actor_d(state.to(self.device),None, h,c,step)
                    if action_softmax.dim() == 1:
                        action_softmax = action_softmax.unsqueeze(0)
                    if index == 0:
