...
class SelectOperations(nn.Module):
    def __init__(self, d_model, operations):
        ...
        # self.selector1 = nn.LSTM(input_size=128, hidden_size=d_model, num_layers=1)
        # self.selector1 = nn.LSTMCell(input_size=128, hidden_size=d_model)
        # self.selector2 = nn.Linear(d_model, operations)
        self.selector = nn.Sequential(
            nn.Linear(d_model, operations),
        )

    def forward(self, enc_outputs, h, c):
        # x = enc_outputs.squeeze()
        # output = self.selector(x)
        # x1 = enc_outputs.unsqueeze(1)
        # out, (h,c) = self.selector1(x1,(h,c))

        # output = out.squeeze()
        # output = self.selector2(output)

        ###########原始的方法
        output = self.selector(enc_outputs)
        return output, h, c
        ###########原始的方法
