...
from Own.ppo_attention.emedding_data.g_mlp import gMLPClassification
from einops import rearrange

from Own.ppo_attention.emedding_data.utils import exists, default
from Own.ppo_attention.emedding_data.shared_classes import Residual, PreNorm
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        with open('tensor_values.txt', 'a') as file:
            for row in x:
                for value in row:
                    file.write(str(value.item()) + '\t')  # 将张量的值以制表符分隔写入文件
                file.write('\n')  # 写入换行符表示新的一行
        x = self.embeds(x)
...
class GatedTabTransformer(nn.Module):
    ...
    def forward(self, x_categ, x_cont=None):
        self.reduce = ReductionDimension(x_categ.shape[0],64)
        self.reduce2 = ReductionDimension(x_cont.shape[0], 512)
        assert x_categ.shape[-1] == self.num_categories, f'you must pass in {self.num_categories} values for your categories input'
        if self.num_categories != 0:
            # x_categ += self.categories_offset

            x = self.transformer(x_categ)
            output_tensor = x.permute(1, 2, 0)
            output_tensor1 = self.reduce(output_tensor.cpu())
            # output_tensor = torch.reshape(output_tensor1, (output_tensor1.shape[0], 512))
            flat_categ = output_tensor1.flatten(1)
        ...
        if self.num_continuous != 0:
            ...
            output_tensor4 = x_cont.permute(1,0)
            output_tensor5 = self.reduce2(output_tensor4.cpu())

        else:
            x = flat_categ

        return flat_categ,output_tensor5
