...
class AutoFE:
    ...
    def fit_attention(self, args):
        ...
        d_v = args.d_v
        d_ff = args.d_ff
        data_features = self.dfs_['FE_train'].shape[1] - 1
        n_heads = args.n_heads
        self.ppo = PPO(args, data_nums,data_features, operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads, self.device)
        ...
        for epoch in range(args.epochs):
            ...
            # XXX = self.workers_top5[0].features[0][-1]
            # XXX['label'] = self.dfs_['FE_train']['label']
            # name = args.file_name
            # XXX.to_csv(name+'.csv',index=False)
            ...
            best_list.append(epoch_best_score)
            print(f"{best_list}********************")
        list_str = str(best_list)
        # 打开文件，以写入模式打开
