...
from Own.controller_lstm_attention_2 import Controller, Controller_binary
...
class PPO_psm(object):
    def __init__(self, args):
        ...
        self.arch_lr = 0.001
        self.episodes = 1
        self.entropy_weight = 1e-1
        self.policy_nums = 100
        ...
        for k in range(self.policy_nums):
            ...
            for unary_worker in tqdm(unary_workers, desc='Unary IDs'):
                for i in range(self.num_step):
                    ...
                    print(self.best_score_list[:15])
                    if score > epoch_best_0:
                        epoch_best_0 = score
                    if score > best_score:
                        best_score = score
            ...
            self.controller = Controller_binary(self.args, unary_workers, self.ori_data).to(self.device)
            binary_workers = self.controller.sample(is_unary=False, args=self.args)
            for binary_worker in tqdm(binary_workers, desc='Binary IDs'):
                for i in range(self.num_step * self.num_step):
            ...
            # update baseline
            sample_unary_score = [worker.score_list for worker in unary_workers]
            sample_binary_score = [worker.score_list for worker in binary_workers]
            sample_score = sample_unary_score[0] + sample_binary_score[0]
            # sample_score.append(sample_binary_score[0])
            # sample_score.append(sample_unary_score[0])
            if self.baseline is None:
                self.baseline = np.mean(sample_score)
            else:
                for score in sample_score:
                    self.baseline = self.baseline * self.baseline_weight + score * (1 - self.baseline_weight)

            score_lists = sample_unary_score[0] + sample_binary_score[0]
            ...
            for ppo_epoch in range(self.ppo_epochs):
                loss = 0
                for unary_worker,binary_worker in zip(unary_workers,binary_workers):
                ...
                loss /= len(unary_workers) + len(binary_workers)
            ...
            epoch_best.append(epoch_best_0)
            print(f"{epoch_best}%%%%%%%%%%%%%%%%%%%%%%%")
            list_str = str(epoch_best)
            # 打开文件，以写入模式打开
