    ...
    def sample(self, is_unary, args):
        ...
        if is_unary:
            ...
            for i in range(self.step_num):
                ...
                print(f"unary{i}", execution_time, "秒")
            ...
            for i in range(self.step_num):
                ...
                if propose_data.shape[1] > 2 * init_data.shape[1]:
                    propose_data = feature_selection_new(propose_data, self.label,
                                     num_features=int(1.5 * init_data.shape[1]),task_type=self.task_type)
...
class Controller_binary(nn.Module):
    def __init__(self, args, workers, ori_data):
        """
               ...
               :param ori_data: 包括label的dataframe
               """
        super(Controller_binary, self).__init__()
    ...
    def sample(self, is_unary, args):
        ...
        for worker in self.workers:
            ...
            ori_states_b = []
            cluster_dict_b = []
            for num, init_data in enumerate(worker.states_u):
                print(init_data.shape)
                start_time = time.time()
                hc_binary_init = self.init_rnn_hidden(is_unary=False)
                print(time.time())
                cluster_dict = cluster_features(init_data, self.label)
                print(time.time())
                emb_input = emb_features(args, init_data, cluster_dict, self.hidden_size)  # shape:(聚类数量，隐藏层数)
                print(time.time())
                for i in range(self.step_num):
                    ...
                    #  根据采样的操作符更新原dataframe
                    print(time.time())
                    propose_data = update_binary(cluster_dict, init_data, binary_actions_name, self.label,
                                                 self.task_type)
                    columns_name = propose_data.columns
                    print(time.time())
                    if propose_data.shape[1] > 2 * init_data.shape[1]:
                        score_funcs = {'reg': f_regression, 'cls': mutual_info_classif}
                        score_func = score_funcs[self.task_type]
                        k_best = 2 * init_data.shape[1]
                        selector = SelectKBest(score_func=score_func, k=k_best)
                        propose_data = selector.fit_transform(propose_data, self.label)
                        selected_features = selector.get_support(indices=True)
                        selected_columns = [columns_name[idx] for idx in selected_features]
                        propose_data = pd.DataFrame(propose_data,columns=selected_columns)
                        # propose_data = feature_selection_new(propose_data, self.label,
                        #                                      num_features=int(1.5 * init_data.shape[1]),task_type=self.task_type)
                    print(time.time())
                    propose_cluster_dict = cluster_features(propose_data, self.label)
            ...
            binary_workers.append(worker_b)

        return binary_workers
