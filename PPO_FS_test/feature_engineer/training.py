...
def sample(args,train_data_x, train_data_y, feature_nums,adj_matrix,ppo, worker, device):
    ...
    actions, log_prob, action_softmax,emb_x = ppo.choose_action(init_x_state, init_y_state,feature_nums,adj_matrix)
    select_emb_x = ppo.feature_select(init_x_state,init_y_state,actions,adj_matrix)
    ...
    worker.emb_x = emb_x
    worker.select_emb_x = select_emb_x
    return worker
