...
from PPO_my2.fe_parsers import parse_actions
from PPO_my2.metric_evaluate import rae_score
from PPO_my2.model_evaluate import model_fuctions
from PPO_my2.process_data import Pipeline
from PPO_my2.process_data.utils_memory import remove_duplication
from PPO_my2.process_data.update_data import Update_data
...
def sample(args, ppo, pipline_args_train, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, epoch, i, device):
    ...
    ori_c_features = df_c_encode.shape[1] -1
    ori_d_features = df_d_encode.shape[1] -1
    ...
    init_con_or_dis = [1] * len(pipline_args_train['continuous_columns']) + [-1] * len(
        pipline_args_train['discrete_columns']) + [0]
    con_or_diss.append(init_con_or_dis)
    for step in range(steps_num):
        if step != 0:
            pipline_ff = Update_data(x_c, x_d, pipline_args_train)
            if x_c.shape[0] != 0:
                n_c_features = x_c.shape[1]
            if x_d.shape[0] != 0:
                n_d_features = x_d.shape[1]
        else:
            pipline_ff = Update_data(df_c_encode, df_d_encode, pipline_args_train)
            if df_c_encode.shape[0] != 0:
                n_c_features = df_c_encode.shape[1]
            if df_d_encode.shape[0] != 0:
                n_d_features = df_d_encode.shape[1]
        steps.append(step)
        state = init_state
        actions_ops, log_ops_probs, emb_state = ppo.choose_action_ops(state, step, epoch, c_ops, init_con_or_dis)

        fe = parse_actions(actions_ops, c_ops, d_ops, df_c_encode, df_d_encode, n_c_features, n_d_features,ori_c_features,ori_d_features)
        ff.append(fe)
        df_c_encode, df_d_encode = pipline_ff.process_data(fe)
        df_encode2 = pd.concat([df_c_encode,df_d_encode],axis=1)
        state2 = torch.from_numpy(df_encode2.values).float().transpose(0, 1).to(device)
        actions_otp, log_otp_probs = ppo.choose_action_otp(state2,emb_state)
        print(sum(actions_otp))
        new_df_c_encode = pd.DataFrame()
        new_df_d_encode = pd.DataFrame()
        for num,action in enumerate(actions_otp):
            if df_c_encode.shape[0] == 0:
                x = 0
            else:
                x = df_c_encode.shape[1]
            if df_c_encode.shape[0] != 0 and 0 <= num < x:
                if action == 1:
                    name = df_c_encode.columns[num]
                    new_df_c_encode[name] = df_c_encode.iloc[:,num]
            elif x <= num:
                if action == 1:
                    name = df_d_encode.columns[num-x]
                    new_df_d_encode[name] = df_d_encode.iloc[:, num-x]
        df_c_encode = pd.concat([new_df_c_encode, df_t_norm], axis=1)
        df_d_encode = pd.concat([new_df_d_encode, df_t_norm], axis=1)
...
def multiprocess_reward(args, worker, c_columns, d_columns, scores_b, mode, model, metric, x_d_onehot, y,
                        df_d_labelencode):
    accs = []
    cvs = []
    scores = []
