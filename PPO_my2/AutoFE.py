...
from PPO_my2.fe_operations import get_binning_df
...
class AutoFE:
    ...
    def fit_attention(self, args):
        ...
        self.ppo = PPO(args, data_nums,feature_nums,operations_c, operations_d, d_model,hidden_size, d_k, d_v, d_ff, n_heads, self.device)
        ...
        for epoch in range(args.epochs):
            ...
            for i, worker in enumerate(workers):
                ...
                for step in range(args.steps_num):
                    ...
                    if sum(worker.scores)/len(worker.scores) > self.best_score:
                        ...
                        xxx = worker_x.states[step].permute(1,0).cpu().numpy()
                        df = pd.DataFrame(xxx)
            ...
            self.ppo.update_ops(workers)
            self.ppo.update_otp(workers)
