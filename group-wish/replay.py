...
class RandomClusterReplay(Replay):
    def __init__(self, size, batch_size, state_dim,device, op_dim=0):
        super(RandomClusterReplay, self).__init__(size, batch_size, device)
        self.memory = np.zeros((self.MEMORY_CAPACITY, state_dim * 2 + state_dim * 2 + op_dim * 2 + 1))
    ...
    def sample(self):
        ...
        b_r = torch.FloatTensor(b_memory[:, self.STATE_DIM + self.ACTION_DIM + 1])
        b_s_ = torch.FloatTensor(
            b_memory[:, self.STATE_DIM + self.ACTION_DIM + 1:self.STATE_DIM * 2 + self.ACTION_DIM + 1])
...
class RandomOperationReplay(Replay):
    def __init__(self, size, batch_size, state_dim, device):
        super(RandomOperationReplay,self).__init__(size, batch_size, device)
...
class PERClusterReplay(RandomClusterReplay):
    def __init__(self, size, state_dim, action_dim, batch_size,device):
        super(PERClusterReplay, self).__init__(size, state_dim, action_dim, batch_size,device)
