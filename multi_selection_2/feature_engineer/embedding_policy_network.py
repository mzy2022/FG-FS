...
class Feature_Build(nn.Module):
    ...
    def forward(self, input_x, input_y):
        global z
        input_x = torch.from_numpy(input_x.values).float().transpose(0, 1).to(self.device)
        input_y = torch.from_numpy(input_y.values).float().transpose(0, 1).to(self.device)
